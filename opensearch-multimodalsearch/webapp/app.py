...
if "input_modelType" not in st.session_state:
    st.session_state.input_modelType = "GPT-J-6B (Sagemaker)"
...
if(search_all_type == True):
    with st.sidebar:
        ...
        sparse = st.checkbox('Expand query and documents with Sparse features', key = 'sparse', help = "Checking this box will use Neural Sparse search. This will expand the queries and documents with semantic sparse features and apply keyword search on the expanded docs and queries. This increases the likelihood of matches overcoming the vocabulary mismatch problem.")
        st.session_state.input_sparse = 'disabled'
